...
describe('combine_spark_output', () => {
  ...
  before(() => {
    if (fs.existsSync(processedDataDir)) {
      fs.readdirSync(processedDataDir)
        .forEach(file => {
          fs.unlinkSync(processedDataDir + file)
        })
    }
  })
  ...
  it('should combine all spark output files into one', (done) => {
    ...
    util.combine_spark_output(
      ...
      let csvFiles = fs.readdirSync(processedDataDir).filter(file => {
        return (path.extname(file) === '.csv')
      })

      // checking that we have csv file generated after combining spark outout
      chai.expect(csvFiles.length).to.be.above(0, `File not found in ${processedDataDir}`)
      ...
      csvData.split(/\n/).forEach((line, index) => {
        ...
        chai.expect(count).to.equal(expectedData.get(origin + destination), `Count did not match for ${origin} - ${destination}`)
      })
      done()
    }).catch(done)
  })
